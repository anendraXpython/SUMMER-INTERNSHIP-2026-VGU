...
import { getListings, getListingLocations, createSavedSearch } from "../api/api";
...
function ListingsSection({ user, onSelectForChatOrCalc }) {
  ...
  const handleSaveSearch = async () => {
    ...
    try {
      const label =
        [location, propertyType, maxPrice ? `under $${Number(maxPrice).toLocaleString()}` : null]
          .filter(Boolean)
          .join(" · ") || "All listings";
      ...
      setSaveSearchMessage("Search saved! We'll email you when new matches come in.");
    } catch (error) {
      setSaveSearchMessage("Could not save this search.");
    }
  };
    ...
    <>
      <section className="hero">
        ...
        <div className="search-box">
          ...
          <button className="btn-secondary save-search-btn" onClick={handleSaveSearch}>
            Save this search
          </button>
        </div>
        {saveSearchMessage && <p className="message success">{saveSearchMessage}</p>}
      </section>
