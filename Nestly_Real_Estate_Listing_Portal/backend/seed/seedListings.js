...
async function seedIfEmpty() {
  ...
  if (listingCount === 0) {
    ...
    for (const listing of SAMPLE_LISTINGS) {
      // created individually (not insertMany) so each one runs through the
      // Listing model's pre-save hook and gets a real priceHistory entry
      await Listing.create({ ...listing, agentUsername: demoAgent.username });
    }

    console.log("Seeded", SAMPLE_LISTINGS.length, "sample listings under demo_agent.");
  }
}
